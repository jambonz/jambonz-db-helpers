...
LEFT JOIN webhooks AS dh ON acc.device_calling_hook_sid = rh.webhook_sid
...
async function lookupAccountBySid(pool, logger, account_sid) {
  ...
  if (r.length > 0) {
    const obj = r[0].acc;
    Object.assign(obj, {registration_hook: r[0].rh, device_calling_hook: r[0].dh});
    if (!obj.registration_hook.url) delete obj.registration_hook;
    if (!obj.device_calling_hook.url) delete obj.device_calling_hook;
